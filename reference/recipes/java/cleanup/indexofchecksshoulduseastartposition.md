...
_Replaces `indexOf(String)` in binary operations if the compared value is an int and not equal to -1._
...
Recipes can also be activated directly from the command line by adding the argument `-DactiveRecipe=org.openrewrite.java.cleanup.IndexOfChecksShouldUseAStartPosition`
